...
/// </summary>
public class Drone_Forest : Agent
{
    ...
    public float distanceBlowupFactor = 2.5f;   // 전역 폭주 방지 배수
    public float minPreDistForBlowup = 3.0f;    // 전역 폭주 최소 임계
    public int   warmupSteps = 20;              // 목표 설정 직후 유예 스텝
    ...
    public override void Initialize()
    {
        ...
        // ✅ RayPerceptionSensorComponent3D 자동 추가/설정
        _ray3d = GetComponent<RayPerceptionSensorComponent3D>();
        if (_ray3d == null)
            _ray3d = gameObject.AddComponent<RayPerceptionSensorComponent3D>();
    ...
    public override void OnEpisodeBegin()
    {
        ...
        // agentTrans.position = startPos + agentTrans.up * 20f + agentTrans.forward * 10f;
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        {
            AddReward(-2f);
            Debug.Log($"[Drone] ⛔ 오버슈트 종료 (d={distance:F2} > min*{overshootFactor:F1}={minDistToTarget * overshootFactor:F2})");
            //EndEpisode();
            return;
        }
        ...
        {
            // 진행 보상(가까워지면 +, 멀어지면 -)
            AddReward((preDist - distance) * 0.01f);
            preDist = distance;
        }
    }
